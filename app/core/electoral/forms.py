    ...
    class Meta:
        ...
        widgets = {
            "denominacion": forms.TextInput(
                attrs={"placeholder": "Ingrese un Departamento"}
            ),
        }
...
class DistritoForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "denominacion": forms.TextInput(
                attrs={"placeholder": "Ingrese un Distrito"}
            ),
        }
...
class SeccionalForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "denominacion": forms.TextInput(
                attrs={"placeholder": "Ingrese una Seccional"}
            ),
        }
...
class PaisForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "denominacion": forms.TextInput(
                attrs={"placeholder": "Ingrese una Ciudad"}
            ),
        }
...
class ManzanaForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "denominacion": forms.TextInput(
                attrs={"placeholder": "Ingrese una Manzana"}
            ),
        }
...
class TipoVotoForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "denominacion": forms.TextInput(
                attrs={"placeholder": "Ingrese una TipoVoto"}
            ),
        }
...
class OperadorForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "denominacion": forms.TextInput(
                attrs={"placeholder": "Ingrese un Operador"}
            ),
        }
...
class ElectorForm(ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if usuario:
            self.fields["barrio"].queryset = Barrio.objects.filter(
                ciudad__distrito=usuario.distrito, activo__exact=True
            )
            self.fields["manzana"].queryset = Manzana.objects.filter(
                barrio__ciudad__distrito=usuario.distrito, activo__exact=True
            )
            self.fields["operador"].queryset = Operador.objects.filter(
                distrito=usuario.distrito, activo__exact=True
            )
    ...
    class Meta:
        ...
        widgets = {
            "ci": forms.TextInput(
                attrs={"placeholder": "Ingrese Cedula", "readonly": "readonly"}
            ),
            "nombre": forms.TextInput(
                attrs={"placeholder": "Ingrese Nombre", "readonly": "readonly"}
            ),
            "apellido": forms.TextInput(
                attrs={"placeholder": "Ingrese Apellido", "readonly": "readonly"}
            ),
            "ciudad": forms.Select(
            ...
            "manzana": forms.Select(
                attrs={"class": "form-control select2", "style": "width: 100%;"}
            ),
            "tipo_voto": forms.Select(
                attrs={"class": "form-control select2", "style": "width: 100%;"}
            ),
            "operador": forms.Select(
                attrs={"class": "form-control select2", "style": "width: 100%;"}
            ),
            "local_votacion": forms.Select(
                attrs={
...
class ElectorForm2(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "ciudad": forms.Select(
                attrs={"class": "form-control select2", "style": "width: 250px;"}
            ),
            "barrio": forms.Select(
                attrs={"class": "form-control select2", "style": "width: 250px;"}
            ),
...
class ShearchForm(forms.Form):
    ...
    term = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": "Buscar por CI, Nombre o Apellido"}
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Mesa nace vacío por defecto (Tu JS se encarga de rellenarlo dinámicamente)
        ...
        if usuario:
            ...
            self.fields["seccional"].choices = [("", "---")] + [
                (item.id, str(item))
                for item in Seccional.objects.filter(
                    ciudad__distrito=distrito, activo=True
            ...
            self.fields["barrio"].choices = [("", "---")] + [
                ...
                for item in Barrio.objects.filter(
                    ciudad__distrito=distrito, activo=True
                ).order_by("id")
            ]
            ...
            self.fields["manzana"].choices = [("", "---")] + [
                (item.id, item.fullname)
                for item in Manzana.objects.filter(
            ...
            self.fields["operador"].choices = [("", "---")] + [
                (item.id, str(item))
                for item in Operador.objects.filter(
        ...
        self.fields["ciudad"].choices = [("", "---")] + [
            (item.id, str(item))
            for item in Ciudad.objects.filter(activo=True).order_by("denominacion")
        ]

        self.fields["tipo_voto"].choices = [("", "---")] + [
            (item.id, str(item))
            for item in TipoVoto.objects.filter(activo=True).order_by("id")
        ]

        self.fields["monto"].choices = [("", "---")] + [
            (item.valor, item.parametro)
            for item in Parametro.objects.filter(
                activo=True, grupo="MTO_GS_DIA_D"
            ).order_by("id")
        ]

        # --- 3. APLICACIÓN DE ESTILOS Y ATRIBUTOS UNIFICADOS ---
        for name, field in self.fields.items():
            if name in ["date_range", "term"]:
                ...
                # Absolutamente todos los ChoiceFields (incluyendo mesa y los pasos por PC/MV/GS)
                # se unifican bajo la clase select2 para mantener la consistencia estética moderna
                field.widget.attrs.update(
